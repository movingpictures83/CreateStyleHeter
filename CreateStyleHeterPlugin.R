...
#nameOfNetwork = "human_ibd_microbiota_genes_metabolites_dbn_sample_alignment_sr14d_top100x100_filtered_dbnIntraBoot100boots.graphml"#list.files(pattern =".*\\.graphml")
#nameOfNetwork = "human_ibd_microbiota_genes_metabolites_dbn_sample_alignment_sr14d_top100x100_host_genes_reference_dbnIntraBoot.graphml"#list.files(pattern =".*\\.graphml")
#nameOfNetwork = paste(folder,nameOfNetwork,sep="/")

# READ STYLE BASE
f <<- readChar(paste(pfix, parameters["styleBase",2], sep="/"), file.info(paste(pfix, parameters["styleBase",2], sep="/"))$size)
